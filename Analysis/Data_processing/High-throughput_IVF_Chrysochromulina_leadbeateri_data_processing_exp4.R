...
data_path <- "../Experiment_nr4_plates"   # path to the data
...
setwd("../Metadata")
expdata <- read_csv2("ToxANoWa_Cleadbeateri_Plate_Variable_values_exp4.csv")
...
data %>% 
  ...
  mutate(Timepassed = int_length(interval(ymd_hms("2022-11-06 18:29:00"), Date_Time))) %>% #Change date (yyyy-mm-dd) and time of first measurement
  ...
  unite("Strain_Experiment", c("Strain","Experiment.y"), sep = "_", remove = FALSE) -> df_final_exp4
df_final_exp4 %>%
  mutate(Day = round(Timepassed)) -> df_final_exp4
